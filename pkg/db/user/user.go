...
// var user User
func (u *userObj) GetUserDetailByID(c *gin.Context) (User, error) {
	logger.Log().Debug("start")
	defer logger.Log().Debug("end")
	//id := c.GetString(config.USERID)
	id := "a"
	query := "SELECT id,mobile_no FROM mylaon.user WHERE id = ?"
	...
	if rows.Next() {
		err := rows.Scan(&user.Id, &user.Mobile_no) // Assuming UserID, Name, Email are the columns in the 'user' table
		if err != nil {
